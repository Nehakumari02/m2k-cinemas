...
import { Business, Visibility, Refresh, GetApp as DownloadIcon } from '@material-ui/icons';
import { downloadSchoolInquiriesCsv } from '../../../utils'; // Can use a generic one or just reuse this if we make a generic one later
...
export default function CorporateGroupInquiryList() {
  ...
  const fetchInquiries = async () => {
    setLoading(true);
    try {
      const response = await fetch(apiUrl('/admin/corporate-group-inquiries'), {
  ...
  const handleSaveStatus = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(apiUrl(`/admin/corporate-group-inquiries/${selected._id}`), {
        method: 'PATCH',
        headers: setAuthHeaders({ 'Content-Type': 'application/json' }),
    ...
    <div className={classes.root}>
      <div className={classes.header}>
        <Typography variant="h4" className={classes.title}>
          <Business style={{ color: '#b72429' }} />
          Corporate Booking Enquiries
        </Typography>
      ...
      {loading ? (
        ...
        <div className={classes.empty}>
          <Business style={{ fontSize: 48, marginBottom: 16, opacity: 0.3 }} />
          <Typography>No corporate booking enquiries yet.</Typography>
        </div>
      ) : (
          ...
          <>
            ...
            <DialogContent>
              <div className={classes.detailRow}>
                <Typography className={classes.detailLabel}>Company</Typography>
                <Typography className={classes.detailValue}>{selected.companyName}</Typography>
              </div>
              <div className={classes.detailRow}>
